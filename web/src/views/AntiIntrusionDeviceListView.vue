...
/**
 * 大机防侵限设备列表（原型《大机防侵限设备列表》+《新增设备》弹窗）。
 *
...
const canAdd = computed(() => auth.hasPermission("anti_intrusion_device:add"));
const canEdit = computed(() => auth.hasPermission("anti_intrusion_device:edit"));
const canDelete = computed(() => auth.hasPermission("anti_intrusion_device:delete"));

const loading = ref(false);
const tableData = ref<AntiIntrusionDevice[]>([]);
const total = ref(0);
const page = ref(1);
const size = ref(10);
...
/** 设备状态标签色：在线=绿 / 不在线=灰 / 低电量=橙，与人机定位设备列表一致 */
...
function projectName(row: AntiIntrusionDevice): string {
...
function buildParams(): AntiIntrusionDeviceListParams {
  return {
...
async function loadData() {
  loading.value = true;
  try {
    const pageData = await fetchAntiIntrusionDevices(buildParams());
...
function fillForm(row: AntiIntrusionDevice) {
...
function openEdit(row: AntiIntrusionDevice) {
  dialogMode.value = "edit";
  fillForm(row);
}

function openView(row: AntiIntrusionDevice) {
  dialogMode.value = "view";
  fillForm(row);
}

function buildData(): AntiIntrusionDeviceCreate {
  return {
  ...
  await formRef.value.validate(async (valid: boolean) => {
    ...
    try {
      ...
      if (dialogMode.value === "create") {
        await createAntiIntrusionDevice(payload);
        ElMessage.success("设备创建成功");
      } else {
        await updateAntiIntrusionDevice(
          editingId.value as number,
          payload as AntiIntrusionDeviceUpdate,
        );
...
async function handleDelete(row: AntiIntrusionDevice) {
  try {
  ...
  try {
    await deleteAntiIntrusionDevice(row.id);
...
const {
  ...
  deleteApi: batchDeleteAntiIntrusionDevices,
  reload: () => loadData(),
  label: "大机防侵限设备",
});
</script>
...
<template>
  <div class="anti-intrusion-device-page">
    <!-- 查询区 -->
    <div class="toolbar">
      <el-select
      ...
      <el-select v-model="query.status" placeholder="设备状态" clearable class="w-140">
        <el-option
          v-for="s in ANTI_INTRUSION_DEVICE_STATUSES"
          :key="s"
          :label="s"
          :value="s"
        />
    ...
    >
      ...
      <template #empty>
        <el-empty description="暂无大机防侵限设备" :image-size="80" />
      </template>
    </el-table>
