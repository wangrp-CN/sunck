...
from app.core.constants import ws_channel_for_correlation, ws_channel_for_project
...
async def correlation_ws(
    websocket: WebSocket,
    token: str | None = Query(default=None, description="访问令牌(JWT)"),
    project_id: int | None = Query(default=None, description="按项目订阅频道"),
) -> None:
    """跨设备共因事件组实时通道：前端订阅后接收新增跨设备共因推送。

    消息体：``{"type":"correlation","action":"new_cross_device","data":<CorrelatedEventGroup>}``。
    频道与告警通道命名空间隔离（``corr:global`` / ``corr:project:N``），
    避免与 /ws/alarm 的 location/alarm 消息混流。
    """
    if not token:
        await websocket.close(code=4401, reason="missing token")
        return
    ...
    channel = ws_channel_for_correlation(project_id)
    await manager.connect(websocket, channel)
    try:
        while True:
            data = await websocket.receive_text()
