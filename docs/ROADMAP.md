...
| ⑧ | **视频 AI 分析** | 重 | P3 | ✅ 已完成(PoC→深化) | 通道/事件模型·接口·回推端点；前端实时拉流播放(HLS/MP4)；`/ai/analyze` 接外部推理端点(超时容错)+能力清单对齐 |
