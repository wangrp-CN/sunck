...
> **进度补记（2026-07-27）**：本表停留在 2026-07-24，期间已额外交付「智能核心 v2」整条链路（风险/健康快照 → 阈值预警 → 跨设备共因关联 → 趋势异常检测 → 异常进告警流 → 一键派单闭环），以及本轮「闭环效能度量」。上述能力均已在 `CHANGELOG` 与 `deploy/README.deploy.md` §13 记录，本表仅补充 ⑬ 与刷新日期。仍待外部依赖：①短信/语音真网关（凭据）、⑧视频AI 推理服务（留桩）。
