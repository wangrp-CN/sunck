"""设备管理 CRUD 与权限/数据隔离测试。

覆盖：
- 管理员完整增删改查（含关键词搜索、软删后详情 404）。
- 无 device:add 权限用户创建被 403 拦截。
- 数据范围隔离：本部门及以下(scope=3)用户仅可见其部门内项目下的设备。

设备按 device_type 派发到 LocateDevice / AntiIntrusionDevice / TrainApproachDevice
三张表，清理时按 device_no 前缀三表统一删除。
按 uid 前缀清理自建数据，避免污染开发库。
"""

from sqlalchemy import delete

from app.core.database import SessionLocal
from app.model.device import (
    AntiIntrusionDevice,
    LocateDevice,
    TrainApproachDevice,
)
from app.model.project import Project
from tests.helpers import (
    _cleanup_org,
    _headers,
    _make_dept,
    _make_role,
    _make_user,
    _uid,
)


def _cleanup(u: str) -> None:
    db = SessionLocal()
    try:
        for m in (LocateDevice, AntiIntrusionDevice, TrainApproachDevice):
            db.execute(delete(m).where(m.device_no.like(f"D{u}%")))
        db.execute(delete(Project).where(Project.name.like(f"P{u}%")))
        db.commit()
    finally:
        db.close()
    _cleanup_org(u)


def _create_project(client, admin_token, u, dept_id, suffix):
    return client.post(
        "/api/v1/projects",
        headers=_headers(admin_token),
        json={"name": f"P{u}_{suffix}", "dept_id": dept_id},
    ).json()["data"]


def test_device_full_crud_by_admin(client, admin_token):
    u = _uid()
    try:
        dept = _make_dept(client, admin_token, u, "A")
        proj = _create_project(client, admin_token, u, dept["id"], "项目")
        # 1) 创建（locate 型）
        r = client.post(
            "/api/v1/devices",
            headers=_headers(admin_token),
            json={
                "device_type": "locate",
                "project_id": proj["id"],
                "name": f"D{u}_定位1",
                "device_no": f"D{u}_LOC1",
                "function": "人机定位",
            },
        )
        assert r.status_code == 200, r.text
        did = r.json()["data"]["id"]

        # 2) 列表 + 关键词搜索
        r = client.get(
            "/api/v1/devices",
            headers=_headers(admin_token),
            params={"keyword": f"D{u}", "page": 1, "size": 20},
        )
        assert r.status_code == 200, r.text
        items = r.json()["data"]["items"]
        assert any(it["id"] == did for it in items)

        # 3) 详情
        r = client.get(f"/api/v1/devices/{did}", headers=_headers(admin_token))
        assert r.status_code == 200, r.text
        assert r.json()["data"]["device_type"] == "locate"

        # 4) 更新
        r = client.put(
            f"/api/v1/devices/{did}",
            headers=_headers(admin_token),
            json={"name": f"D{u}_定位改"},
        )
        assert r.status_code == 200, r.text
        assert r.json()["data"]["name"] == f"D{u}_定位改"

        # 5) 软删后详情 404
        r = client.delete(f"/api/v1/devices/{did}", headers=_headers(admin_token))
        assert r.status_code == 200, r.text
        r = client.get(f"/api/v1/devices/{did}", headers=_headers(admin_token))
        assert r.status_code == 404
    finally:
        _cleanup(u)


def test_device_create_requires_permission(client, admin_token):
    u = _uid()
    try:
        dept = _make_dept(client, admin_token, u, "A")
        proj = _create_project(client, admin_token, u, dept["id"], "项目")
        role = _make_role(client, admin_token, u, "READER", ["device:list"])
        tok = _make_user(client, admin_token, u, f"T{u}_reader", role["code"], dept["id"])

        r = client.post(
            "/api/v1/devices",
            headers=_headers(tok),
            json={
                "device_type": "locate",
                "project_id": proj["id"],
                "name": f"D{u}_无权限",
                "device_no": f"D{u}_DENY",
            },
        )
        assert r.status_code == 403, r.text
    finally:
        _cleanup(u)


def test_device_data_isolation_by_dept(client, admin_token):
    u = _uid()
    try:
        dept_a = _make_dept(client, admin_token, u, "A")
        dept_b = _make_dept(client, admin_token, u, "B")
        proj_a = _create_project(client, admin_token, u, dept_a["id"], "A")
        proj_b = _create_project(client, admin_token, u, dept_b["id"], "B")
        role = _make_role(
            client,
            admin_token,
            u,
            "DM",
            ["device:list", "device:add", "device:edit", "device:delete"],
            data_scope=3,
        )
        tok = _make_user(client, admin_token, u, f"T{u}_dm", role["code"], dept_a["id"])

        da = client.post(
            "/api/v1/devices",
            headers=_headers(admin_token),
            json={
                "device_type": "locate",
                "project_id": proj_a["id"],
                "name": f"D{u}_A",
                "device_no": f"D{u}_A",
            },
        ).json()["data"]
        db_dev = client.post(
            "/api/v1/devices",
            headers=_headers(admin_token),
            json={
                "device_type": "anti_intrusion",
                "project_id": proj_b["id"],
                "name": f"D{u}_B",
                "device_no": f"D{u}_B",
            },
        ).json()["data"]

        r = client.get("/api/v1/devices", headers=_headers(tok))
        assert r.status_code == 200, r.text
        ids = {it["id"] for it in r.json()["data"]["items"]}
        assert da["id"] in ids
        assert db_dev["id"] not in ids

        r = client.get(f"/api/v1/devices/{db_dev['id']}", headers=_headers(tok))
        assert r.status_code == 404

        r = client.post(
            "/api/v1/devices",
            headers=_headers(tok),
            json={
                "device_type": "locate",
                "project_id": proj_a["id"],
                "name": f"D{u}_A2",
                "device_no": f"D{u}_A2",
            },
        )
        assert r.status_code == 200, r.text
    finally:
        _cleanup(u)
